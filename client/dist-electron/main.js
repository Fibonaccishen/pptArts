...
function getSettings() {
  try {
    const settingsPath = require$$1$1.join(require$$1$3.app.getPath("userData"), "settings.json");
    if (require$$1.existsSync(settingsPath)) {
      return JSON.parse(require$$1.readFileSync(settingsPath, "utf-8"));
    }
  } catch {
  }
  return {};
...
function setupUpdater() {
  ...
  if (settings.serverUrl) {
    ...
    mainExports.autoUpdater.setFeedURL({ provider: "generic", url: feedUrl });
  }
}
...
mainExports.autoUpdater.on("update-not-available", () => {
  mainWindow == null ? void 0 : mainWindow.webContents.send("update-status", { status: "not-available" });
});
