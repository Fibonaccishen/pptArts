...
| 场景 | 操作 | 下一次表现 |
|------|------|-----------|
| 不下载 | 弹窗"是否下载"时点 **暂不更新** | 下次启动继续提醒"是否下载" |
| 下载后不安装 | 下载完成后点 **稍后** | 下次启动/手动检查 → 直接提示"是否立即重启"（**不重新下载**） |
| 错过更新 | 没看到弹窗直接用了 | 点顶栏"检查更新"按钮手动触发 |
...
下载好的安装包缓存在用户电脑本地，**不会因关闭应用而丢失**。选择"稍后"后，下次启动或手动检查会直接跳到安装提示，无需重新下载。
